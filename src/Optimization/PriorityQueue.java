...
public class PriorityQueue {
   ...
   public void resize() {
      if (heap.length <= length + 1) {
         ...
         for (int i = 0; i < length; i++)
            for (int j = 0; j < 3; j++)
               temp[i][j] = heap[i][j];
         for (int i = length-1; i < heap.length; i++)
            heap[i][0] = Integer.MAX_VALUE;
         heap = temp;
      }
   ...
   public void heap(int index) {
      // checks that the item isn't on the end, w/o this getleft & getRight
      ...
      if (!(index >= (length / 2))) {
         ...
         if (heap[index][0] > heap[getLeft(index)][0]
            ...
            // switches the places of the item and its smaller child. then
            // reruns the method.
   ...
   public void add(int weight, int index, int parentIndex) {
      ...
      length++;
      resize();
      heap[length][0] = weight; // heap[i][0] stores the weight of the edge in
                                // the graph
      heap[length][1] = index; // heap[i][1] stores the index of the node in the
                               // heap
      heap[length][2] = parentIndex; // heap[i][2] stores the index of the
                                     // node's parent in the heap
      int runner = length;
